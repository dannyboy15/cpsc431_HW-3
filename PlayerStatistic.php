...
class PlayerStatistic
{
  ...
  function name() {
    ...
    // void name($first_name, $last_name)
        ...
        $this->name['FIRST'] = sanitize(func_get_arg(0));
        $this->name['LAST']  = sanitize(func_get_arg(1)); 
    }
  ...
  function __construct($name="", $time="0:0", $points=0, $assists=0, $rebounds=0) {
    ...
    // Note, can't check for "true" because strpos() only returns the boolean
    // value "false", never "true"
    if( strpos($name, "\t") !== false) { 
      // assign each argument a value from the tab delineated string
      // respecting relative positions
  ...
  // Returns a tab separated value (TSV) string containing the contents of all
  // instance attributes   
  function toTSV() {
    return implode("\t", [$this->name(), $this->playingTime(),
      $this->pointsScored(), $this->assists(), $this->rebounds()]);
  }
  ...
  // Sets instance attributes to the contents of a string containing ordered,
  // tab separated values 
  function fromTSV(string $tsvString) {
    // assign each argument a value from the tab delineated string respecting
    // relative positions
