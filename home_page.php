...
<body>
                ...
                <td>
                  <select name="name">
                    ...
                    while($row = mysqli_fetch_array($result_names_all, MYSQLI_ASSOC)) {
                      if ($row["Name_First"] == 'NULL' || $row["Name_First"] == "" ) {
                        printf ("<option value=\"%s\">%s</option>\n",
                          $row["Name_Last"], $row["Name_Last"]);
                      } else {
 ...
 <div class="display">
   ...
   T.Country,
   T.ZipCode,
   AVG(S.PlayingTimeMin),
   AVG(S.PlayingTimeSec),
   AVG(S.Points),
   AVG(S.Assists),
   AVG(S.Rebounds),
   COUNT(S.ID)
   ...
   <table class="align-top table-bordered" style="border:1px solid black; border-collapse:collapse;">
    ...
    while($row = mysqli_fetch_array($result_stats_all, MYSQLI_ASSOC)) {
      ...
      // Name cell
      $lname      = $row["Name_Last"];
      $fname      = $row["Name_First"] == 'NULL' ? "" : $row["Name_First"];
      $commaname  = $fname == "" ? "" : ", ";
      ...
      $commaaddr  = ($city == "") || ($state == "") ? "" : ", ";
      $zipCode    = $row["ZipCode"] == 0 ? "" : $row["ZipCode"];
      $country    = $row["Country"] == 'NULL' ? "" : $row["Country"];


      // Convert data to strings or empty string if 0
      $NumGames     = $row["COUNT(S.ID)"];
